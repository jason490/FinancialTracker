...
// MergeCategories merges source category into target category, verifying ownership
func MergeCategories(db *sql.DB, userID int64, sourceID int64, targetID int64) error {
	tx, err := db.Begin()
	if err != nil {
		return err
	}
    ...
    var ownerID1, ownerID2 int64
    if err := tx.QueryRow(`SELECT user_id FROM categories WHERE id = ?`, sourceID).Scan(&ownerID1); err != nil {
        return err
    }
    if err := tx.QueryRow(`SELECT user_id FROM categories WHERE id = ?`, targetID).Scan(&ownerID2); err != nil {
        return err
    }
    if ownerID1 != userID || ownerID2 != userID {
        return sql.ErrNoRows
    }

	// Find colliding tags
	rows, err := tx.Query(`
		SELECT name FROM tags WHERE category_id = ? 
		INTERSECT 
		SELECT name FROM tags WHERE category_id = ?`, sourceID, targetID)
	if err != nil {
		return err
	}
	...
	for _, name := range collidingNames {
		...
		err = tx.QueryRow(`SELECT id FROM tags WHERE category_id = ? AND name = ?`, sourceID, name).Scan(&sourceTagID)
		if err != nil { return err }
		err = tx.QueryRow(`SELECT id FROM tags WHERE category_id = ? AND name = ?`, targetID, name).Scan(&targetTagID)
		if err != nil { return err }

		// Move transaction mappings from source tag to target tag
		if _, err := tx.Exec(`UPDATE OR IGNORE transaction_tags SET tag_id = ? WHERE tag_id = ?`, targetTagID, sourceTagID); err != nil {
			return err
		}
		
		// Move filters
		if _, err := tx.Exec(`UPDATE OR IGNORE tag_filters SET tag_id = ? WHERE tag_id = ?`, targetTagID, sourceTagID); err != nil {
			return err
		}

		// Delete the source tag since it's merged
	...
	// Move remaining tags
	if _, err := tx.Exec(`UPDATE tags SET category_id = ? WHERE category_id = ?`, targetID, sourceID); err != nil {
		return err
	}

	// Delete source category
	if _, err := tx.Exec(`DELETE FROM categories WHERE id = ? AND user_id = ?`, sourceID, userID); err != nil {
		return err
	}
