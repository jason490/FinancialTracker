...
| `SUBSCRIPTIONS_ENABLED` | When `true` (default), Stripe billing and per-tier usage limits are active. Set to `false` to disable paid plans, Stripe checkout, and quota enforcement. When `false`, new account registration requires a single-use invite code from an administrator. |
| `REGISTRATION_ADMIN_EMAILS` | Semicolon-separated emails allowed to issue registration invite codes via `POST /api/v1/admin/registration-codes` when subscriptions are disabled (e.g. `admin@example.com;ops@example.com`). In development, the seeded `test@test.com` user is always a registration admin. Bootstrap codes without a signed-in admin: `make registration-code`. |
...
*   **Configure:** `cp env.prod.example .env` and set `SITE_HOST`, `API_PUBLIC_URL`, `FRONTEND_URL` (HTTPS), and secrets.
*   **Build images:** `make prod-build` (compiled Go binary + static SSG frontend baked into Caddy image). Rebuild after changing `API_PUBLIC_URL` or `FRONTEND_URL`.
*   **Start stack:** `make prod-up` (uses `docker-compose.prod.yml`, persistent `db_data` volume, no hot-reload). Two runtime services: **Caddy** (automatic Let's Encrypt TLS, `/api` proxy, static file server) and **Go API**.
*   **Stop stack:** `make prod-down`
*   **Logs:** `make prod-logs`
*   Set `SITE_HOST` to your bare domain (e.g. `app.example.com`). Caddy provisions and renews TLS automatically when DNS points at the server and ports 80/443 are open. Set `API_PUBLIC_URL` and `FRONTEND_URL` to matching `https://` URLs. Optional `ACME_EMAIL` registers with Let's Encrypt for certificate expiry notices. Dev stack remains `docker compose up` with `docker-compose.yml`.
